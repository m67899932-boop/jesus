    ...
    <AuthGuard>
      <section>
        <div className="flex justify-between items-center mb-4">
          ...
          <Link href="/os/new"><a className="px-3 py-2 bg-slate-900 text-white rounded">Nova OS</a></Link>
        </div>
        <div className="grid gap-3">
          {list.length===0 && <div className="text-sm text-slate-500">Nenhuma OS cadastrada</div>}
          {list.map(o=>(
            <div key={o.id} className="p-3 bg-white border rounded">
              <div className="flex justify-between">
                <div>
                  <div className="font-medium">{o.title}</div>
                  <div className="text-xs text-slate-500">Criada em: {new Date(o.created).toLocaleString()}</div>
                </div>
