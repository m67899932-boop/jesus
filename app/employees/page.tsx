      ...
      <section>
        ...
        <div className="mb-4 p-4 bg-white border rounded">
          <div className="flex gap-2">
            <input value={name} onChange={e=>setName(e.target.value)} className="flex-1 p-2 border rounded" placeholder="Nome"/>
            <button onClick={add} className="px-3 py-2 bg-slate-900 text-white rounded">Adicionar</button>
          </div>
        </div>
        <div className="grid gap-3">
          {list.map(emp=>(
            <div key={emp.id} className="p-3 bg-white border rounded flex justify-between">
              <div>
          ...
          {list.length===0 && <div className="text-sm text-slate-500">Nenhum funcionário</div>}
        </div>
      </section>
    </AuthGuard>
  )
